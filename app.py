...
CANONICAL_KEYS = {
    ...
    "non product box": "non-product_box",
    "non_product_box": "non-product_box",
...
def load_search_terms_with_true_headers(file) -> pd.DataFrame | None:
    ...
    'search term' or 'row labels' and treat that as header row. Works for CSV and Excel.
    """
    try:
        kind = _ext(file.name)
        if kind == "csv":
...
def canonicalize_col(col: str) -> str | None:
    ...
    if key:
        return key
    # fallbacks: remove double underscores, normalize hyphens/underscores
    col2 = col.replace("__", "_").replace("-", "_")
    return CANONICAL_KEYS.get(col2)
